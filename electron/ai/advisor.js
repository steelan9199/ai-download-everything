...
async function ask(prompt, options = {}) {
  ...
  if (!cfg.apiKey) return { ok: false, error: '还没配置大模型 API Key，去「设置」页填一下。' };
  ...
  try {
    res = await fetch(url, {
      ...
      body: JSON.stringify({ model: cfg.model || 'gpt-4o-mini', messages, temperature: options.temperature ?? 0.2 })
    });
  } catch (e) {
...
function buildScriptPrompt({ url, host, diagnostic, userDesc }) {
  return [
...
module.exports = { ask, buildScriptPrompt, getCallCount };
