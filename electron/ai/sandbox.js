...
export const ALLOWED_BINS = ["ffmpeg", "ffmpeg.exe", "yt-dlp", "yt-dlp.exe"];
...
function resolvePath(workDir, p, fullAccess) {
  ...
  if (path.isAbsolute(p))
    throw new PermissionDenied("绝对路径 " + p + " 超出了下载目录", {
      path: p,
    });
  const root = path.resolve(workDir);
  const abs = path.resolve(root, p);
  if (abs !== root && !abs.startsWith(root + path.sep))
    throw new PermissionDenied("路径 " + p + " 越出下载目录", { path: p });
  return abs;
}
...
export async function runScript(opts = {}) {
  ...
  const api = {
    ...
    fs: {
      writeFile: (p, data) =>
        fs.writeFileSync(
          resolvePath(workDir, p, fullAccess),
          Buffer.isBuffer(data) ? data : String(data),
        ),
      readFile: (p) =>
        fs.readFileSync(resolvePath(workDir, p, fullAccess), "utf8"),
      list: () =>
        fs.readdirSync(resolvePath(workDir, ".", false)).map((n) => n),
    ...
    exec: async (bin, args) => {
      ...
      if (!fullAccess && !allowed)
        throw new PermissionDenied("想运行未授权命令：" + bin, { bin });
      let resolved = bin;
  ...
  try {
    const context = vm.createContext(
      { api, console: { log, error: log } },
      { codeGeneration: { strings: false, wasm: false } },
    );
    const script = new vm.Script(`(async () => { ${opts.code} })()`, {
    ...
    if (fs.existsSync(rp)) {
      try {
        resultJson = JSON.parse(fs.readFileSync(rp, "utf8"));
      } catch (_) {
        resultJson = fs.readFileSync(rp, "utf8");
      }
    }
    return {
      ok: true,
