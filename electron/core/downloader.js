...
async function downloadSegments(segments, opts = {}) {
  ...
  async function worker() {
    while (cursor < segments.length) {
      ...
      try {
        const buf = await downloadOne(seg.uri, dest, { headers, maxRetries, timeoutMs });
        results[idx] = { ...seg, local: dest, ok: true, size: buf.length };
      } catch (e) {
        results[idx] = { ...seg, local: dest, ok: false, error: String(e.message || e) };
  ...
  const workers = [];
  for (let i = 0; i < Math.min(concurrency, Math.max(1, segments.length)); i++) workers.push(worker());
  await Promise.all(workers);

  return { results, errors, allOk: results.length > 0 && results.every((r) => r && r.ok) };
}

/** 下载单个分段到内存并原子落盘，带重试；已完整存在则跳过（断点续传） */
async function downloadOne(uri, dest, { headers = {}, maxRetries = 3, timeoutMs = 20000 } = {}) {
...
module.exports = { downloadSegments, downloadOne };