...
function runSpawn(bin, args, { onLine = () => {}, timeLimitMs = 0 } = {}) {
  return new Promise((resolve, reject) => {
    const cp = spawn(bin, args, { windowsHide: true });
    let stderr = '';
    let stdout = '';
    const timer = timeLimitMs > 0 ? setTimeout(() => { try { cp.kill('SIGKILL'); } catch (_) {} }, timeLimitMs) : null;
    cp.stdout.on('data', (d) => { stdout += d.toString(); });
    cp.stderr.on('data', (d) => { stderr += d.toString(); onLine(d.toString()); });
...
async function mergePlaylist(ffmpegPath, playlistFile, outputFile, { onLine = () => {} } = {}) {
  // -allowed_extensions ALL：允许 key/分段用非标准扩展名（.key/.bin/.m4s 等）
  const args = ['-y', '-allowed_extensions', 'ALL', '-i', playlistFile, '-c', 'copy', '-movflags', '+faststart', outputFile];
  await runSpawn(ffmpegPath, args, { onLine });
  return outputFile;
}

/** 纯 TS 裸分段合并（不经过 m3u8）：用 concat demuxer，需要一个列表文件 */
async function concatList(ffmpegPath, listFile, outputFile, { onLine = () => {} } = {}) {
 ...
 */
async function probeDuration(ffmpegPath, file, { onLine = () => {} } = {}) {
  const { stderr } = await runSpawn(ffmpegPath, ['-i', file], { onLine }).catch((e) => ({ stderr: e.message }));
  const m = /Duration:\s*(\d+):(\d+):(\d+\.?\d*)/.exec(stderr);
  if (!m) return null;
  return parseInt(m[1], 10) * 3600 + parseInt(m[2], 10) * 60 + parseFloat(m[3]);
}

module.exports = { runSpawn, mergePlaylist, concatList, probeDuration };