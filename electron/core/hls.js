...
export function resolveUri(baseUrl, uri) {
  if (/^https?:\/\//i.test(uri)) return uri;
  return new URL(uri, baseUrl).toString();
}

export function isMasterPlaylist(parsed) {
  return parsed.isMaster || parsed.playlists.length > 0;
}

/** 从 master 子流里选码率最高的一路（普通人是「要最清晰」，直接选带宽最高） */
export function pickBestVariant(parsed) {
  const list = parsed.playlists
    .slice()
    .sort(
      (a, b) =>
        (parseInt(b.attrs.BANDWIDTH, 10) || 0) -
        (parseInt(a.attrs.BANDWIDTH, 10) || 0),
    );
  return list[0] || null;
}
