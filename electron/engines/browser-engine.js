...
class BrowserInterceptEngine {
  ...
  isCancelled() { return this.cancelled || !!(this.cancelToken && this.cancelToken.cancelled); }

  cancel() { this.cancelled = true; try { this.win && this.win.close(); } catch (_) {} }
  ...
  async run(url) {
    ...
    const onReq = (details, cb) => {
      if (/\.m3u8(\?.*)?$/i.test(details.url)) {
        if (!m3u8List.includes(details.url)) { m3u8List.push(details.url); this.onEvent({ type: 'found-m3u8', url: details.url }); }
      } else if (/\.(ts|m4s|mp4|key)(\?.*)?$/i.test(details.url)) {
        if (!seenSeg.has(details.url)) { seenSeg.add(details.url); segUrls.push(details.url); }
      }
      cb({});
    };
    ...
    if (!m3u8Url) {
      const manual = await this.askUser({
        id: 'paste-m3u8',
        prompt: '程序没抓到 m3u8 链接。请用「猫抓」或 F12→网络 里复制一条 .m3u8 链接粘到下面（越多画质越好）；没有就留空，我会尝试边播边存：',
        options: [
    ...
    if (m3u8Url) {
      ...
      if (!segFiles || segFiles.length === 0) return this.fallbackEdgePlay(segUrls, ctx);
  ...
  async tryAutoPlay() {
    const selectors = [
      'video', 'button[class*="play"]', 'button[id*="play"]', '.vjs-big-play-button',
      '[class*="plyr__control--overlaid"]', 'div[class*="play-icon"]', 'button[aria-label*="播"]'
    ];
  ...
  async grabPlaylist(m3u8Url, ctx, round = 0) {
    let text = await http.getText(m3u8Url, { headers: this.buildHeaders(ctx, m3u8Url) });
    if (text.status === 403 || text.status === 401) {
      // 会话过期：让真人重新过验证后复用新 Cookie
      await this.askUser({ id: 'session-expired', prompt: '会话可能过期了（403）。请回到浏览器窗口刷新/重新过验证，然后点下面的「继续」。', options: [{ value: 'retry', label: '已刷新，继续' }] });
      const ctx2 = await this.collectContext(m3u8Url);
      text = await http.getText(m3u8Url, { headers: this.buildHeaders(ctx2, m3u8Url) });
    ...
    if (hls.isMasterPlaylist(parsed)) {
      ...
      const sub = await http.getText(subUrl, { headers: this.buildHeaders(ctx, subUrl) });
    ...
    if (keyRef && keyRef.key.uri) {
      ...
      keyBuf = await http.getBuffer(keyUrl, { headers: this.buildHeaders(ctx, keyUrl) });
    ...
    if (parsed.map && parsed.map.uri) {
      ...
      const mapBuf = await http.getBuffer(mapUrl, { headers: this.buildHeaders(ctx, mapUrl) });
    ...
    const jobs = parsed.segments.map((s, i) => ({
      ...
      seq: s.seq, duration: s.duration, key: s.key, map: s.map, idx: i
    }));
  ...
  async fallbackEdgePlay(segUrls, ctx) {
    if (segUrls.length === 0) throw new Error('既没抓到 m3u8 也没有分片，请检查视频是否在播放');
    this.onEvent({ type: 'edge-play', count: segUrls.length });
    const jobs = segUrls.map((u, i) => ({ uri: u, fileName: `seg_${String(i).padStart(6, '0')}.ts`, seq: i }));
    const { results } = await downloader.downloadSegments(jobs, {
      outDir: this.workDir, headers: this.buildHeaders(ctx, segUrls[0]), concurrency: this.concurrency,
      onProgress: (p) => this.onEvent({ type: 'progress', ...p })
    });
  ...
  async merge(segFiles) {
    ...
    if (parsed.targetDuration) lines.push(`#EXT-X-TARGETDURATION:${Math.ceil(parsed.targetDuration)}`);
    if (this.keyFile) {
      // AES：本地 key；若按序号 IV，补上 IV
      const keyIv = parsed.segments && parsed.segments.find((s) => s.key && s.key.iv);
      const ivStr = (keyIv && keyIv.key && keyIv.key.iv) ? `,IV=0x${keyIv.key.iv.toString('hex')}` : '';
      lines.push(`#EXT-X-KEY:METHOD=AES-128,URI="${path.basename(this.keyFile)}"${ivStr}`);
    }
    if (this.mapFile) lines.push(`#EXT-X-MAP:URI="${path.basename(this.mapFile)}"`);
    ...
    await ffmpeg.mergePlaylist(this.ffmpegPath, playlistFile, outFile, { onLine: (l) => this.onLog({ level: 'info', msg: l.trim() }) });
    this.onEvent({ type: 'merge-done', file: outFile });
    return outFile;
  }

  close() { try { this.win && this.win.close(); } catch (_) {} }
}

/** 从捕获到的 m3u8 列表里挑最像媒体流的（优先带 .m3u8 且非广告前缀）。简单策略：取第一个 */
function pickM3u8(list) { return list.length ? list[0] : null; }

/** 解析 Copy as cURL 命令：抽出 URL + 请求头（Cookie/Referer/UA 等） */
function parseCurl(cmd) {
  const urlMatch = /curl\s+(?:-[^\s]*\s+)*['"]?(https?:\/\/[^'"]+)['"]?/.exec(cmd) || /['"](https?:\/\/[^'"]+\.m3u8[^'"]*)['"]/.exec(cmd);
  const headers = {};
  const hRe = /-H\s+['"]([^'"]+)['"]/g;
  let m;
...
module.exports = { BrowserInterceptEngine, parseCurl };