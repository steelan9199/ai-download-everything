...
import * as sandbox from "../ai/sandbox.js";
...
async function resolveFailure(url, err, opts) {
  ...
  const ans = await askUser({
    id: "diagnose",
    prompt:
      "下载没成功。请看一眼窗口/页面，选一个最接近的情况（也可点「其他」补充一两句）：",
    options: [
  ...
  if (
    ...
    return {
      ok: false,
      needRetry: true,
      error: "请回到窗口完成（登录 / 播放 / 刷新）后，再点一次下载。",
    };
  }
  // 其它：转纯人工指引
  return {
    ok: false,
    error:
      "试了内置办法没成功，建议：用「猫抓/F12」复制 .m3u8 链接粘给程序，或配置 API Key 让 AI 诊断。",
  };
}
...
async function aiRescue(url, context, opts) {
  ...
  if (!aiCfg.apiKey)
    return {
      ok: false,
      error: "还没配置大模型 API Key，去「设置」页填一下再重试。",
      engineUsed: "ai",
    };
  ...
  const ai = await advisor.ask(
    advisor.buildScriptPrompt({ url, host, diagnostic: context, userDesc: "" }),
    {
      system:
        "你是下载器的诊断脚本助手。只输出一段 ```javascript 代码或 SOLUTION_IMPOSSIBLE，不要解释。",
    },
  );
  ...
  if (!code)
    return {
      ok: false,
      error: "大模型判断无法自动解决，请走人工：用猫抓复制 m3u8 链接重试。",
      engineUsed: "ai",
    };

  onEvent({ type: "status", msg: "拿到脚本，正在受限沙箱里执行……" });
  const res = await runAiScript({
    code,
    downloadDir,
    tools,
    settings,
  ...
  if (res.ok) {
    ...
    siteRules.sediment({
      id: "ai-" + host,
      host,
      match: escapeRegExp(host),
      kind: "script",
      script: code,
      note: "AI 自动沉淀 " + new Date().toISOString(),
    });
    return {
      ok: true,
      output: res.output,
      engineUsed: "ai",
      aiCalls: advisor.getCallCount(),
    };
  }
  return {
    ok: false,
    error: res.error || "AI 脚本未成功产出结果",
    engineUsed: "ai",
    aiCalls: advisor.getCallCount(),
  };
}
...
async function runAiScript({
  ...
  if (res.hasPermissionDenied) {
    ...
    const g = await askUser({
      ...
      options: [
        { value: "grant", label: "临时授权，继续执行" },
        { value: "deny", label: "拒绝，中止" },
      ],
    });
    if (g.choice !== "grant")
      return { ok: false, error: "已拒绝越界授权，任务中止" };
    res = await run(true);
  }
  if (!res.ok)
    return {
      ok: false,
      error: res.error || res.reason?.message || "脚本执行失败",
    };

  const output = pickOutput(res.resultJson, downloadDir);
  if (!output)
    return { ok: false, error: "脚本跑完了但没写下明确产物，请看下载目录。" };
...
function pickOutput(rj, dir) {
  ...
  try {
    for (const f of fs.readdirSync(dir)) {
      ...
      if (
        Date.now() - st.mtimeMs < 5 * 60 * 1000 &&
        (!best || st.mtimeMs > best.mtimeMs)
      )
        best = full;
    }
  } catch (_) {
...
export function buildDiagnostic(url, msg) {
  return `URL: ${url}\n错误: ${msg}`;
}
