...
export async function download(url, opts = {}) {
  ...
  try {
    ...
    return { ok: false, hint: ["DIR"], error: String(e.message || e) };
  }
  ...
  await new Promise((resolve) => {
    ...
    cp.on("error", (e) => {
      ...
      result.hint = ["BIN"];
      onLog({ level: "error", msg: "yt-dlp 启动失败: " + result.error });
      resolve();
    });
    cp.on("close", (code) => {
      if (code === 0) {
...
export function guessHint(stderr) {
  const s = String(stderr || "");
  const hits = [];
  if (/cloudflare|cf_clearance|bot fight|just a moment/i.test(s))
  ...
  if (/unable to download|no video formats|unsupported url/i.test(s))
    hits.push("NO_FORMATS");
