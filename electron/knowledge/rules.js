...
export const RULES = [
  {
    ...
    engine: "browser",
    advice:
      "这个站有 Cloudflare 反爬，普通直连会被挡。已改用电浏览器（内嵌 Chromium）打开，通常会自己通过人机校验；若弹出验证，你在窗口里点一下即可。",
    fix: [
      "用内嵌浏览器打开页面并复用 Cookie（cf_clearance）",
      "过验证后让视频播放，程序自动拦截 m3u8",
    ],
  },
  {
    id: "HTTP403",
    ...
    engine: "browser",
    advice:
      "服务器返回 403（拒绝访问），多半是防盗链或反爬。已切到浏览器拦截，若还 403，多半是会话 Cookie 过期。",
    fix: [
      "浏览器里刷新页面/重新过验证，让 Cookie 更新",
      "或用「猫抓/F12」手工复制一条 m3u8 链接粘给程序",
    ],
  },
  {
    id: "M3U8",
    ...
    engine: "browser",
    advice:
      "这是 HLS 视频（m3u8 分片）。若普通工具只抓到几秒，通常是「直播式滑窗」防盗链，程序会持续刷新 playlist 拿全部分段。",
    fix: [
      "自动按内置逻辑并发抓取全部分段并 ffmpeg 合并",
      "拿不全时用猫抓复制具体画质的 m3u8 链接重试",
    ],
  },
  {
    id: "NEED_AUTH",
    ...
    engine: "browser",
    advice:
      "这视频需要先登录（或会员）。请在窗口里登录后，再点播放让程序抓到流。",
  ...
  },
  {
    ...
    engine: "browser",
    advance:
      "yt-dlp 在这个页面找不到可下载的视频流。可能 URL 是列表页/需要登录，或该站太特殊。",
  },
  {
    id: "BIN",
    ...
    fix: [
      "设置页里「浏览」找到 yt-dlp.exe",
      "或在管理员 PowerShell 里执行 winget install yt-dlp.yt-dlp",
    ],
  },
  {
    id: "DIR",
